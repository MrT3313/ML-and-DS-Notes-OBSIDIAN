...
| **Scalability** | That it go on doing so as the load on it grows: [[Scalability]]. | Load grows on a third axis here, beside complexity and traffic volume, which is the number of models, and it grows because one application has many features and each may need a model of its own. One model detects trending hashtags, then another filters unsafe content, then another filters bot-generated posts, and in the limit there is one model per customer, so the model count $m$ equals the customer count $n$, $m = n$. Named companies are far past one or two. Uber's Michelangelo platform had "hundreds of use cases representing thousands of models" in production at any given time (Del Balso and Hermann, Uber Engineering, November 2018). Booking.com reports many pages carrying several models each, and studies about 150 customer-facing applications that succeeded in controlled trials (Bernardi, Mavridis and Estevez, KDD 2019), which counts successes and not everything deployed. Scale is also the ordinary case and not a big-company exception, though the figure usually quoted for it proves less than it sounds: in the 2019 Stack Overflow Developer Survey, 53.1% of the 71,791 respondents who gave a company size worked at an organization of 100 or more employees, but the respondents were developers in general, only 7.9% of whom counted themselves data scientists or machine learning specialists, and headcount is not user count. Artifact management is the second half of that growth. Monitoring, retraining and reproducing models requires automation as soon as there is at least one model in production, $m \geq 1$, and the claim is the threshold rather than the merit of automation: it is one model, not many. |

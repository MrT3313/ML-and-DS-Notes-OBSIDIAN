...
- **A miscalibrated model.** Every uncertainty measure above is a function of $P_{\theta}(y \mid \mathbf{x})$ and assumes those numbers track how often the model is actually right. A model whose scores are monotone but not calibrated ranks instances in an order that has nothing to do with their informativeness, and the loop is then reading noise with great precision. This bites hardest at the start, where the cold start guarantees poor calibration.

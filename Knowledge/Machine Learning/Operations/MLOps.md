...
- **[[Model Inference]]**, running a fitted model on an input with every weight fixed, and what that costs: latency read at a high percentile of an end-to-end sum in which the network, the queue and the feature fetch sit beside the model's own time, and throughput, which a bulk call raises at the price of every row waiting for the whole call.
- **[[Batch and Online Prediction]]**, when a prediction is computed, ahead of any request and stored or on the request itself, trading a stored answer's bounded staleness and the work wasted on rows nobody asks for against a latency budget paid on every call.
- **[[Model Compression]]**, making a fitted model smaller so it costs less to store, ship and run, by lowering the parameter count, the bytes per parameter or the number of values actually stored, at an accuracy cost that is measured rather than assumed. Its four techniques each have a note: [[Low-Rank Factorization]] and [[Knowledge Distillation]] cut the parameter count, the first inside a layer and the second by replacing the model with a smaller student, [[Model Pruning]] cuts the parameters or the stored nonzeros depending on whether it removes whole units or single weights, and [[Model Quantization]] cuts the bits per parameter.

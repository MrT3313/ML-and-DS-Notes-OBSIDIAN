...
- Deployment targets, half-supplied. The vocabulary for where anything at all runs now exists in `Knowledge/Data Systems/Deployment/`: [[Cloud Computing]] and [[Self-Hosting]] for who owns the machines, [[Serverless]] for renting the execution rather than the machine, and [[Cloud-Native Architecture]] for what a service has to look like to suit either. What is still missing is the model-specific half, where a trained *model* runs once it leaves a notebook and what serving one asks that serving an ordinary service does not. Chapter 19, Training and Deploying at Scale, is the chapter that brings this and most of the rest of this list.

...
- Launch, monitor and maintain is the last section of the chapter and it produced nothing at all, which makes it the largest gap here. `Knowledge/MLOps/` currently holds its hub and a single concept note, [[Model Rot]]. What else does that folder need before it is a real area: deployment targets, monitoring and alerting on live inputs, a retraining cadence, rollback, and where does the confidence interval on a shipped model's error belong?

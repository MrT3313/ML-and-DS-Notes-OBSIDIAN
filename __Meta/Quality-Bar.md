...
- An index states what is on disk, and a forward-looking gap list is not part of the shape. A list of material whose arrival is predicted from chapters not yet read is a claim made on the weakest evidence available, and it goes stale silently, since nothing breaks when the prediction turns out wrong.
